...
def grepObject(t0, t1):
    ...
    if(len(areas)>0):
        ...
        if(areas[max_index]>4000):
            ...
            pts.append(center)    
            if(len(pts)>0):
                accux=accux+pts[-1][0]-pts[0][0]
                accuy=accuy+pts[-1][1]-pts[0][1]
                #cv2.line(t1, pts[-1], pts[0], (0, 250, 253), 10)
                ...
                if abs(accux)>xth and abs(accuy)<yth:
                    if accux>xth:
                        ...
                        hand_direction='left'
                    #cv2.line(t1, (pts[-1][0],pts[0][1]), (pts[0][0],pts[0][1]), (0, 250, 253), 10)
                    end=True
                    ...
                    if accuy>yth:
                        ...
                        hand_direction='up'
                    #cv2.line(t1, (pts[-1][1],pts[0][0]), (pts[0][1],pts[0][0]), (0, 250, 253), 10)
                    end=True
                elif abs(accux)>xth and abs(accuy)>yth:
